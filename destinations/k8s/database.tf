      ...
      spec {
        ...
        container {
          ...
          command = ["bash", "-c", join(" && ", [
            ...
            "install -v -d -m 0777 -o ${local.app_uid} -g ${local.app_gid} ${local.managed_config_dir}",
            "python3 ${local.root_dir}/scripts/create_db.py -c ${local.config_dir}/galaxy.yml install"
          ])]
          image             = "${local.galaxy_app_image}:${var.image_tag}"
          image_pull_policy = "Always"
          env {
